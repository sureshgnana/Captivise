...
class BootstrapModalLink(tables.Column):
    ...
    def render(self, value, record):
        return format_html('<a href="javascript:void(0)" onclick="getScriptStatus({value})" data-id="{value}">Script Status</a>', value=value)


class PaymentTable(tables.Table):
    transaction_id = tables.Column(verbose_name='Transaction ID')
    status = tables.Column(verbose_name='Status')
    currency = BudgetColumn(verbose_name='Currency')
    amount = ConversionTypeColumn(verbose_name='Amount')
